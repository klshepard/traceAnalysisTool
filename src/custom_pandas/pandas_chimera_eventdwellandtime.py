...
def pandas_the_IV():
    """
    Plots event dwelltimes for files that are tagged "DNA".  TODO, check stuff for automatically sorting out DNA length
    ...
    eventsDF = eventsDF[
        eventsDF.EVENTDWELLTIME < 100e3  # in microseconds, so that's 100 millisec
    ...
    IVplot = sns.scatterplot(
        x="EVENTDWELLTIME", y="EVENTDEPTH", data=eventsDF, hue="VAC"
    )
    ...
    #     alpha=0.5,
    # )
    plt.xlabel(r"Event dwell time [microsec]")
    plt.ylabel(r"Event depth" + " [{su}]".format(su=statsDF["unit"].iat[0]))
    fileName = sample + "_eventdepthandtime.png"
