...
if __name__ == "__main__":
    ...
    resultsPath = Path("/proj/jakobuchheim/share/newChunk/Device_3")
    ...
    axlimitlist = [40, 8, 8, 120, 20, 50, 12, 1]
    ...
    for i in boxplotlist:
        ...
        ax = sns.boxplot(
            x="ident", y=i, data=statsDF, hue="conc", ax=axsns, dodge=False
        )
    ...
    sns.relplot(x="conc", y="K_OFF_double_fast", data=sub2)
    sns.relplot(x="conc", y="K_ON_double_fast", data=sub2)
    # sns.scatterplot(x="conc", y='EVENTSPERSEC', data=statsDF)#[(statsDF['sliceTime'] == 60 & statsDF['type'].str.contains('srtn'))], ax = axsns)

    # ax.set_xticklabels(ax.get_xticklabels(), rotation=70)

    figsns.tight_layout()
    fileName = "K_fast_vs_conc.png"
    figsns.savefig(savePath / fileName, dpi=200)
    figsns.clear()
    plt.close(figsns)
    ...
    stdlist = boxplotlist
    stdlist.extend(["SAMPLE", "type", "sliceTime"])
    ...
    for group in subDF.groupby(["SAMPLE", "type", "conc", "sliceTime"]):
        ...
        # statsDF[statsDF['SAMPLE'] == group[1]['SAMPLE'] and statsDF['type'] == group[1]['type'] and statsDF['conc'] == group[1]['conc'] and statsDF['sliceTime'] == group[1]['sliceTime']]['MLH_K_ON_FAST'] = averageWeighted
    ...
    for group in subDF.groupby("ident"):
        ...
        print(stdWeighted - group[1]["K_ON_double_fast"].std())
        print(group[0])
        statsDF[statsDF["ident"] == group[0]].loc[:, "MLH_K_ON_FAST"] = averageWeighted
