...
def my_exp_zero(x_in, m1, m2, b):
    # Linear, two-sloped fit to current
    # BUG this is a gross implementation
    out = []
    if isinstance(x_in, int) or isinstance(x_in, float):
        if x_in > 0:
            out.append(m1 * x_in + b)
        elif x_in < 0:
            out.append(m2 * x_in + b)
        else:
            out.append(b)
    else:
        for x in x_in:
...
def my_exp_linear(x_in, m1, m2, x0, y0):
    # Linear, two-sloped fit to current
    out = []
    x0 = float(x0)
    y0 = float(y0)
    if isinstance(x_in, int) or isinstance(x_in, float):
        if x_in > x0:
            out.append(m1 * x_in + y0)
        elif x_in < x0:
            out.append(m2 * x_in + y0)
        else:
            out.append(y0)
    else:
        for x in x_in:
...
def error_function_single(popt, T, I):
    assert len(T) == len(I)
    error = np.linalg.norm(I - my_exp(T, *popt))
    return error


def fit_single_linear(x, y):
    params = scipy.optimize.fmin(
        error_function_single, [10, 10, 0, 0], args=(x, y), xtol=1e-6, maxiter=10000
    )
    return params


def fit_single_exponent(x, y):
    params = scipy.optimize.fmin(
        error_function_single, [1, 0.2, -2], args=(x, y), xtol=1e-3, maxiter=10000
    )
    return params
...
# utility plot functions -- the below should be called by a plot function in main, on a correct synthesis of groupby..
def snsIVplot(df, savepath):
    sns.set()
    sns.set_style("whitegrid")
    plt.figure(figsize=(8, 8))

    IVplot = sns.scatterplot(x="VAC", y="I_mean", data=df[1], hue="COMMENTS", zorder=2)
    IVplot.errorbar(
        df[1].VAC,
        df[1].I_mean,
        yerr=df[1].I_std,
        elinewidth=0.5,  # width of error bar line
        ecolor="k",  # color of error bar -- at some point, should figure out how to have the error bar the same color as the dot...
    ...
    # reposition legend
    box = IVplot.get_position()
    IVplot.set_position(
        [box.x0, box.y0, box.width, box.height * 0.85]
    )  # resize position
    IVplot.legend(loc="upper center", bbox_to_anchor=(0.5, 1.25), ncol=2)

    plt.xlabel(r"$V_{AC}$ [mV]")
    plt.ylabel(r"$I_{A}$ [pA]")

    fileName = df[0][0] + "_salt_" + str(df[0][1]) + "MKCl_IV.png"
    IVplot.figure.savefig(savepath / fileName, dpi=600)
    plt.close()
    return


def snsIVplotregression(df, savepath):
    sns.set()
    sns.set_style("whitegrid")
    plt.figure(figsize=(12, 8))
    IVplot = sns.lmplot(x="VAC", y="I_mean", data=df[1], hue="COMMENTS")
    # lmplot returns a FacetGrid, which is not something I fully understand (fig, ax, axes)?  This causes some shenanigans with legend and the like, but you get the fit line for free.
    plt.xlabel(r"$V_{AC}$ [mV]")
    plt.ylabel(r"$I_{A}$ [pA]")

    fileName = df[0][0] + "_salt_" + str(df[0][1]) + "MKCl_IV_fit.png"
    IVplot.savefig(savepath / fileName, dpi=600)
    plt.close()
    return


def snsIVgateplot(df, savepath):
    sns.set()
    sns.set_style("whitegrid")
    plt.figure(figsize=(8, 8))
    IVplot2 = sns.scatterplot(x="V_g_norm", y="R", hue="COMMENTS", data=df[1], zorder=2)
    ...
    fileName = df[0][0] + "_salt_" + str(df[0][1]) + "MKCl_IV_gate.png"
    IVplot2.figure.savefig(savepath / fileName, dpi=600)
    plt.close()
    return
...
if __name__ == "__main__":
    ...
    logging.info("pandas_utils is saving the previous run to resultsArchive")
    resultsStoragePath = Path.cwd().parent / "ChimeraData/resultsArchive"
    ...
    #     files_for_model = sorted(resultsPath.rglob('*_modelframe.csv'))
    #     for ex in results_frame[results_frame['exp_name'].str.contains('pindown')].groupby(['SAMPLE','conc', 'COMMENTS']):
    ...
    #                 logging.info("Found a model to load.")
    #                 model = pd.read_csv(name, usecols=[1, 2, 3, 4, 5, 6])
    #                 A = model.loc[(model['device'] == ex_device) & (model['conc'] == ex_conc)]['A']
    #                 x0 = model.loc[(model['device'] == ex_device) & (model['conc'] == ex_conc)]['x0']
    #                 B = model.loc[(model['device'] == ex_device) & (model['conc'] == ex_conc)]['B']
    #                 sub_cur = []
    #                 for v in df['VAC']:
    ...
    #         the_ax.grid(which='minor', linewidth='0.5', color='gray', alpha = 0.5)
    #         the_ax.grid(which='major', linewidth='1', color='gray', alpha = 0.5)
    #         the_ax.plot(df['VAC'].to_numpy()/1000, df['I_mean'].to_numpy()/1000, '.', label='$I_{A}$ [nA]', alpha=0.5)
    #         the_ax.plot(df['VAC'].to_numpy()/1000, sub_cur, '.', label='Model $I_{inject}$ [nA]', alpha = 0.5)
    #         the_ax.plot(df['VAC'].to_numpy()/1000, df['I_mean'].to_numpy()/1000 - sub_cur, '.', label='$I_{pore}$ [nA]', markersize = 10)
    #         fileName = ex[0][0]+'_salt_'+str(ex[0][1])+'_MKCl_'+ex[0][2]+'_IVAC_model.png'
    #         plt.xlabel(r"$V_{AC}$")
    #         the_ax.legend()
    ...
    # files_for_model = sorted(resultsPath.rglob('*_modelframe.csv'))
    # for ex in results_frame[results_frame['exp_name'].str.contains('pindown')].groupby(['SAMPLE','conc', 'COMMENTS', 'VAC']):
    #     df = ex[1][['VAC', 'I_mean', 'I_gate', 'vg']]
    #     df['vg_halfcell'] = df['VAC']/2-df['vg'] # anode minus gate.
    ...
    #     the_ax.grid(which='minor', linewidth='0.5', color='gray', alpha = 0.5)
    #     the_ax.grid(which='major', linewidth='1', color='gray', alpha = 0.5)
    #     the_ax.plot(df['vg'].to_numpy()/1000, sub_cur, '.', label='Model $I_{inject}$ [nA]', alpha = 0.5)
    #     the_ax.plot(df['vg'].to_numpy()/1000, df['I_mean'].to_numpy()/1000, '.', label='$I_{A}$ [nA]', alpha=0.5)
    #     the_ax.plot(df['vg'].to_numpy()/1000, df['I_gate'].to_numpy()/1000, '.', label='$I_{gate}$ [nA]', alpha = 0.5)
    #     the_ax.plot(df['vg'].to_numpy()/1000, df['I_mean'].to_numpy()/1000 - df['I_gate'].to_numpy()/1000, '.', label='$I_{pore}$ [nA]', markersize = 10)
    #     fileName = ex[0][0]+'_salt_'+str(ex[0][1])+ex[0][2]+'_VAC_'+str(ex[0][3])+'MKCl_IVg_subtraction.png'
    #     plt.xlabel(r"$V_{AG}$")
    #     the_ax.legend()
    #     the_fig.savefig(Path.cwd().parent/'ChimeraData/results'/ fileName, dpi=600)
    #     plt.close()
    ...
    #     the_ax.plot(df['vg'].to_numpy()/1000, df['I_mean'].to_numpy()/1000, '.', label='$I_{A}$ [nA]', alpha=0.5)
    #     the_ax.plot(df['vg'].to_numpy()/1000, sub_cur, '.', label='Model $I_{inject}$ [nA]', alpha = 0.5)
    #     the_ax.plot(df['vg'].to_numpy()/1000, df['I_mean'].to_numpy()/1000 - sub_cur, '.', label='$I_{pore}$ [nA]', markersize = 10)
    #     fileName = ex[0][0]+'_salt_'+str(ex[0][1])+ex[0][2]+'_VAC_'+str(ex[0][3])+'MKCl_IVg_model.png'
    #     plt.xlabel(r"$V_{AG}$")
    #     the_ax.legend()
    #     the_fig.savefig(Path.cwd().parent/'ChimeraData/results'/ fileName, dpi=600)
    #     plt.close()
    ...
    #     snsIVplot(ex, Path.cwd().parent/'ChimeraData/results')
    #     snsIVplotregression(ex, Path.cwd().parent/'ChimeraData/results')
    ...
    for ex in results_frame[results_frame["exp_name"].str.contains("pindown")].groupby(
        ...
        print(ex)
        snsIVgateplot(ex, Path.cwd().parent / "DataAndResults/results")
    exit()
    ...
    for ex in results_frame[results_frame["exp_name"].str.contains("pindown")].groupby(
        ...
        IVplot2.figure.savefig(
            Path.cwd().parent / "ChimeraData/results" / fileName, dpi=600
        )
        plt.close()
...
if __name__ == "__main":
    ...
    dataPath = Path(Path.cwd().parent / "DataAndResults/dump")
    ...
    assert len(files_for_frame) == 1, "clean dataframes?"
    ...
    results_frame["R"] = results_frame["VAC"] / results_frame["I_mean"]
    results_frame["R"] = (
        results_frame["R"] * 1000
    )  # mV/pA implies gigaohms for the R units here, so multiply by 1000 to get Mohm
    results_frame["V_g_norm"] = (
        2 * results_frame["VAG"] / results_frame["VAC"]
    )  # units of unity, off v_ac/2 is gate off

    logging.info(results_frame.info())

    # g = sns.pairplot(results_frame.dropna(), hue='conc', vars=['V_g_norm', 'I_gate', 'I_mean', 'I_std', 'R'])
    # g.savefig(Path.cwd().parent/'ChimeraData/results' / 'pairplot.png', dpi=600)
    # plt.close()
